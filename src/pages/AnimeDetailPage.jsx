...
function ErrorReportButton({ seriesId, title, lang }) {
  ...
  const CATEGORIES_VI = ['Thông tin sai', 'Ảnh bìa lỗi', 'Link xem hỏng', 'Thiếu tập', 'Nội dung vi phạm', 'Khác']
  const CATEGORIES_EN = ['Wrong info', 'Bad cover image', 'Broken watch link', 'Missing episode', 'Inappropriate content', 'Other']
  ...
  const popup = open ? createPortal(
    <div onClick={close} style={{ position:'fixed', inset:0, zIndex:50000, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(6px)', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'linear-gradient(145deg,#04101a,#060d1a)', border:`1px solid rgba(239,68,68,0.3)`, borderRadius:18, padding:24, width:'100%', maxWidth:380, boxShadow:'0 24px 60px rgba(0,0,0,0.9)' }}>
        {sent ? (
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
            ...
            <button onClick={close} style={{ background:'none', border:'none', color:'#1e3a4a', fontSize:20, cursor:'pointer', lineHeight:1 }}>×</button>
          </div>
          ...
          <button onClick={submit} disabled={!category||sending} style={{ width:'100%', padding:'11px 0', borderRadius:10, border:'none', background:category?'linear-gradient(135deg,#dc2626,#b91c1c)':'rgba(255,255,255,0.06)', color:category?'#fff':'#1e3a4a', fontSize:14, fontWeight:700, cursor:category?'pointer':'not-allowed', fontFamily:"'Be Vietnam Pro',sans-serif", boxShadow:category?'0 4px 16px rgba(220,38,38,0.4)':'none', transition:'all 0.2s' }}>
        ...
        <div style={{ position:'relative', zIndex:2, padding: isMobile ? '48px 16px 28px' : '52px 32px 40px', paddingLeft: isMobile ? 16 : 228,
          ...
          <div style={{ flex:1, paddingTop: isMobile ? 32 : 16 }}>
            ...
            <div style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'center' }}>
              ...
              <ErrorReportButton seriesId={anime.id} title={title} lang={lang} />
