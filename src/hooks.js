...
export function useSeriesStats(seriesId) {
  ...
  useEffect(() => {
    ...
    // Count view once per browser session per series
...
export function useUserRating(seriesId, userToken, userId, onRated) {
  ...
  const submitRating = async (stars) => {
    ...
    const value = Math.round(stars * 2) / 2
    if (value < 0.5 || value > 5) return
    setSaving(true)
    try {
      const body = { series_id: seriesId, rating: value }
      if (userId) body.user_id = userId
      const res = await fetch(`${SUPABASE_URL}/rest/v1/series_ratings`, {
        method: 'POST',
        headers: {
          apikey:        SUPABASE_ANON,
          Authorization: `Bearer ${userToken}`,
      ...
      if (res.ok) {
        setRating(value)
        setHovered(0)
        if (onRated) onRated()  // trigger stats refresh
      }
    } catch (e) {
